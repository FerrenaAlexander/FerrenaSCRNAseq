...
#' #' @param mad.score.threshold_iterative Numeric. Maximum number of deviations from median tolerated before outlier classification. Default = 2.5. lower is more strict, higher is more lenient.
...
#'   if( missing( mad.score.threshold_iterative )) {mad.score.threshold_iterative <- 1}
#'
...
#'     Command = c("mad.score.threshold", "mad.score.threshold_iterative",
...
#'
#'
#'     Option = c(mad.score.threshold, mad.score.threshold_iterative,
#'                baselinefilter, baselinefilter.complexity, baselinefilter.libsize, baselinefilter.mito,
#'                clusters, removemitohiclust,
#'                iterativefilter, iterativefilter.complexity, iterativefilter.libsize, iterativefilter.mito)
#'
...
#'         # get lib size cutoffs; use a right/upper tail only
#'         # right tail of Z-score, solve for which value matches the
#'         iterative.libsize.cutoff.lo <- median(x) - (mad.score.threshold_iterative  * mad(x) )
...
#'         # get percent mito cutoff; use a right/upper tail only
#'         # right tail of Z-score, solve for which value matches the
#'         iterativefilter.mito.cutoff <- median(x) + (mad.score.threshold_iterative  * mad(x) )
#'
#'
#'         #get cells above the threshold
