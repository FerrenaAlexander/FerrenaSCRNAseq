...
seuratpipeline <- function(data,
  ...
  tmp@commands$ferrenascpipeline[['pre_iterative-filter_vln']] <- VlnPlot(tmp, c('percent.mito', 'nCount_RNA', 'nFeature_RNA'), ncol=1, pt.size = 0.1)
  ...
  if(removemitomaxclust == T){
    ...
    if( length(mitohiclusts) != 0 ) {
      ...
      tmp@commands$ferrenascpipeline[['mitohiclust-result']] <- VlnPlot(tmp, c('percent.mito', 'nCount_RNA', 'nFeature_RNA'), ncol=1, pt.size = 0.1)


    } else{
      message('\nNo Mito High clusters identified by Grubbs test\n')

      tmp@commands$ferrenascpipeline[['mitohiclust-result']] <- paste0("No Mito High clusters identified by Grubbs test")
  ...
  if(iterativefilter == T) {
    ...
    tmp@commands$ferrenascpipeline[['iterativefilter_results']] <- reportingdf

    tmp@commands$ferrenascpipeline[['iterativefilter_details']] <- finalparamdf

    tmp <- tmp[,filteredcells]

    tmp@commands$ferrenascpipeline[['iterativefilter_post-vln']] <- VlnPlot(tmp, c('percent.mito', 'nCount_RNA', 'nFeature_RNA'), ncol=1, pt.size = 0.1)
  ...
  names(dratedf) <- c('MultipletProportion', 'CellsLoaded_100%Viability', 'CellsRecovered')

  dbmodel <- lm(MultipletProportion ~ CellsRecovered, data = dratedf)
  ...
  tmp@commands$ferrenascpipeline[['DoubletFinder_results']] <- dfoutput
